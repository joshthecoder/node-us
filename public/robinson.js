function RobinsonProjection() {
  ...
  1., 0., 0., 0
    ...
    this.poly = function(targetArray, offset, z) {
        return (targetArray[offset] + z * (targetArray[offset+1] + z * (targetArray[offset+2] + z * targetArray[offset+3])));
    }
    ...
    this.project = function(longitude, latitude) {
        var lplam = this.degreesToRadians(longitude);
        var lpphi = this.degreesToRadians(latitude);
        var phi = Math.abs(lpphi);
        var i = Math.floor(phi * C1);
        if (i >= NODES) {
            i = NODES - 1;
        }
        phi = this.radiansToDegrees((phi - RC1 * i));
        i *= 4;
        var x = this.poly(X, i, phi) * FXC * lplam;
        var y = this.poly(Y, i, phi) * FYC;
        if (lpphi < 0.0)
            y = -y;
        return [(x / (2.6662696851016574 * 2)) + 0.5, 1 - ((y / (1.352299930694625 * 2)) + 0.5)];
    }
}
