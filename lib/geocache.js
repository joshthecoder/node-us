...
function replaceState(text) {
    ...
    for (var i = 0; i < stateCodes.length; i++) {
        var nextText = text.replace(stateCodes[i][1], stateCodes[i][0]);
        if (newText != text) {
            return nextText;
        }
    }
...
function replaceCountry(text) {
    ...
    for (var i = 0; i < countryCodes.length; i++) {
        var nextText = text.replace(countryCodes[i][1], countryCodes[i][0]);
        if (newText != text) {
            return nextText;
        }
    }
...
function tryGet(query, callback) {
    client.get(query.hash, function(err, value) {
        ...
        if (query.transformations.length > 1) {
            ...
            var newQuery = t(query.original);
            if (newQuery !== undefined) {
                sys.log(query + "\t\t=>\t" + newQuery);
                query.hash = hashLocation(newQuery);
                tryGet(query, callback);
            }
        }
...
exports.store = function(location, coordinates) {
    ...
    client.set(hash, coordinates[0] + " " + coordinates[1],
        function(err) {
            if (err) {
